...
Dataset
The dataset comprises 4049 isolated cell images that are taken from Pap Smear slides (Cropped), of different sizes. For that, and for the fact that EfficientNetB0 requires a specific input size of 224x224x3, a pre-processing step of resizing each image into 224x 224x3 was done. Each isolated cell image belongs to one of the 5 classes: Dyskeratotic (Abnormal), Koilocytotic (Abnormal), Metaplastic (Benign), Parabasal (Normal), and Superficial-Intermediate (Normal).
